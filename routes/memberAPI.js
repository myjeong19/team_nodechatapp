  ...
  try {
    ...
    if (!member) {
      ...
      if (await bcrypt.compare(currentPW, member.member_password)) {
        ...
        var test = await db.Member.update(updateMember, {
        ...
        return res.json({
          code: 200,
          data: member.name,
          resultMsg: "password change success!",
        });
      } else {
        return res.json({
          code: 400,
          data: null,
          resultMsg: "Password is not correct",
        });
      }
    }
  } catch (err) {
    return res.json({
      code: 500,
      data: null,
      resultMsg: "Server ERROR in /api/member/password/update",
    });
  }
});
...
router.post("/add", async (req, res, next) => {
  ...
  try {
    ...
    if (!member) {
      ...
      member.dataValues.email_de = aes.decrypt(
        member.email,
        process.env.MYSQL_AES_KEY
      );
      apiResult.code = 200;
  ...
  //console.log("before send apiReuslt ", apiResult);
  res.json(apiResult);
});
